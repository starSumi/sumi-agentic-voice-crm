...
test("workbench keeps synchronous request state mutually exclusive with recording", () => {
  ...
  assert.match(script, /if \(requestInFlight \|\| capturePending \|\| isRecording\(\)\) return/);
  ...
  assert.match(script, /addEventListener\("stop"/);
});

test("workbench discriminates every generated response envelope before completion", () => {
  for (const guard of ["isAskPayload", "isReviewPayload", "isReviewDecisionPayload", "isErrorPayload"]) {
...
test("protected audio uses the generated authenticated content operation and revokes object URLs", () => {
  ...
  assert.match(script, /getAssetContent\(\{[\s\S]*path: \{ asset_id: asset\.asset_id \}[\s\S]*parseAs: "blob"/);
