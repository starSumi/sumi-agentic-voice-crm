...
test("mutating intents always require review regardless of provider flag", () => {
  ...
  assert.equal(
    requiresReview(understanding("crm.deal.update_stage", false)),
    true,
  );
  assert.equal(
    normalizeUnderstanding(understanding("crm.customer.create", false))
      .needs_confirmation,
    true,
  );
});
...
test("read-only intents continue to honor provider confirmation", () => {
  ...
  assert.equal(
    normalizeUnderstanding(understanding("crm.search", false))
      .needs_confirmation,
    false,
  );
  assert.equal(
    normalizeUnderstanding(understanding("crm.search", true))
      .needs_confirmation,
    true,
  );
});
...
test("unknown intents fail closed as mutating", () => {
  ...
  assert.equal(
    normalizeUnderstanding(understanding("crm.future_command", false))
      .needs_confirmation,
    true,
  );
});
