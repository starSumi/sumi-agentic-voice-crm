...
test.before(async () => {
  ...
  jwksServer = createServer((_req, res) => {
    ...
    res.end(JSON.stringify({ keys: [{ ...publicJwk, alg: "RS256", kid: "auth-test", use: "sig" }] }));
  });
...
async function token(overrides = {}, signingKey = privateKey) {
  return await new SignJWT({ tenant_id: "tenant-a", scope: "voice-crm.write", ...overrides })
...
function authenticator(overrides = {}) {
  return createAuthenticator({ env: {
    APP_ENV: "test",
    AUTH_MODE: "oidc",
    OIDC_ISSUER: issuer,
...
test("OIDC authentication verifies signature, claims, tenant and scope", async () => {
  const identity = await authenticator()(new Headers({ authorization: `Bearer ${await token()}`, "x-tenant-id": "tenant-a" }));
  assert.deepEqual(identity, { tenant_id: "tenant-a", actor_id: "actor-a", auth_mode: "oidc", token_id: undefined, algorithm: "RS256" });
});

test("OIDC authentication rejects forged, mismatched and underscoped tokens", async () => {
  const authenticate = authenticator();
  const attacker = await generateKeyPair("RS256");
  await assert.rejects(authenticate(new Headers({ authorization: `Bearer ${await token({}, attacker.privateKey)}`, "x-tenant-id": "tenant-a" })), (error) => error.code === "UNAUTHORIZED");
  await assert.rejects(authenticate(new Headers({ authorization: `Bearer ${await token()}`, "x-tenant-id": "tenant-b" })), (error) => error.code === "FORBIDDEN");
  await assert.rejects(authenticate(new Headers({ authorization: `Bearer ${await token({ scope: "voice-crm.read" })}`, "x-tenant-id": "tenant-a" })), (error) => error.code === "FORBIDDEN");
