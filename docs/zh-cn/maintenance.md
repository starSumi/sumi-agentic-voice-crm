...
| 当前会话 ID、agent 关系、重试、锁和临时证据 | 依次使用 `CODEX_HOME`、`XDG_STATE_HOME`、平台本地应用数据目录 | 机器本地状态；`pnpm run agent:resume` 会输出实际路径，`.agent-runtime/` 只作为显式后备。 |
...
`.agent/.agent.maintainers.json` 维护文档、连续性和 CI 运维的责任，`CODEOWNERS` 把相同范围路由给仓库所有者。`pnpm run agent:health` 校验责任记录、评审周期和控制面一致性。
...
本地快照丢失或损坏时，只删除对应 thread 的外部 operations 目录，再运行 `pnpm run agent:resume`。版本化 cursor 是恢复来源。治理变更有误时应 revert 对应的独立 commit，不能把运行时历史写回 `.agent/`。
