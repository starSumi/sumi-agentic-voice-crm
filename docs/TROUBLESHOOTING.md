...
Include command, exit code, Node/pnpm versions, relevant request ID, sanitized response, affected document or endpoint, expected behavior, and whether rollback was attempted. Never attach tokens, customer audio, raw transcripts, or absolute local paths containing sensitive names.
