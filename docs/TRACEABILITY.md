...
| Tenant isolation and audit | `docs/SECURITY.md`, OIDC/JWKS verifier, migration RLS policies, `/v1/events` scope | signed JWT issuer/audience/tenant/scope tests plus `pnpm run test:postgres` two-tenant matrix, encrypted interaction replay and authenticated event read; staging IdP and full C5 review still required |
