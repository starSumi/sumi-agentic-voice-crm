...
function setting(env: ProviderEnvironment, name: string, alias?: string, fallback?: string): string | undefined {
  return env[name] || (alias ? env[alias] : undefined) || fallback;
}

function messageContent(body: Record<string, any>): string | undefined {
...
function nativeTtsUrl(compatibleBaseUrl: string): string {
  return new URL("/api/v1/services/aigc/multimodal-generation/generation", compatibleBaseUrl).toString();
}

function languageType(locale: string): string | undefined {
  if (locale === "zh-CN") return "Chinese";
  if (locale === "en-US") return "English";
  return undefined;
}

function audioHostSuffixes(env: ProviderEnvironment): string[] {
...
function assertAllowedAudioUrl(rawUrl: string, env: ProviderEnvironment): URL {
  let url;
...
function verifiedMimeType(declaredType: string | null | undefined, bytes: Buffer): string {
  const declared = declaredType?.split(";", 1)[0].trim().toLowerCase();
  const magic = audioMagicType(bytes);
  const detected = ({ wav: "audio/wav", mp3: "audio/mpeg", ogg: "audio/ogg", mp4: "audio/mp4", webm: "audio/webm" } as Record<string, string | undefined>)[magic ?? ""];
...
function decodeAudioData(raw: unknown, maxBytes: number): { bytes: Buffer; mimeType: string } {
...
async function downloadAudio(initialUrl: string, { env, fetchImpl, signal }: { env: ProviderEnvironment; fetchImpl: FetchImplementation; signal?: AbortSignal }): Promise<{ bytes: Buffer; mimeType: string }> {
...
export function createDashScopeProvider({ env, fetchImpl }: { env: ProviderEnvironment; fetchImpl: FetchImplementation }): ProviderAdapter {
  ...
  return {
    ...
    async synthesize(text, { language = "zh-CN", voice = "default", format = "wav", signal }) {
      ...
      const input: { text: string; voice: string; language_type?: string } = { text, voice: selectedVoice };
