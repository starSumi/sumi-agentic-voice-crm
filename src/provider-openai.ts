...
export function createOpenAiProvider({ env, fetchImpl }: { env: ProviderEnvironment; fetchImpl: FetchImplementation }): ProviderAdapter {
  ...
  return {
    ...
    async transcribe(audio, { locale, contentType = "audio/wav", signal }) {
      ...
      form.append("file", new Blob([new Uint8Array(audio)], { type: uploadType }), `input.${extension}`);
      form.append("model", model);
      form.append("language", locale.split("-")[0]);
      const response = await fetchImpl(`${baseUrl()}/audio/transcriptions`, { method: "POST", headers: headers(undefined), body: form, signal });
