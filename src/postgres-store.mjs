...
import { DataCipher } from "./data-cipher.mjs";
...
export class PostgresCrmStore {
  ...
  async health() {
    try { await this.pool.query("select 1"); return { ready: true, provider: "postgres" }; }
  ...
  async #transaction(identity, work, { requireActor = true } = {}) {
    ...
    try {
      ...
      const tenant = await client.query("select id from tenants where id = $1 and status = 'active'", [identity.tenant_id]);
      if (tenant.rowCount !== 1) throw Object.assign(new Error("tenant is not active"), { code: "FORBIDDEN" });
      let actorUuid = null;
      if (requireActor) {
        const actor = await client.query("select id from actors where tenant_id = $1 and subject = $2", [identity.tenant_id, identity.actor_id]);
        if (actor.rowCount !== 1) throw Object.assign(new Error("actor is not registered in tenant"), { code: "FORBIDDEN" });
        actorUuid = actor.rows[0].id;
      }
      const result = await work(client, actorUuid);
  ...
  async execute({ tenant_id, actor_id, idempotency_key, intent, entities, request_id, request_fingerprint }) {
    ...
    return await this.#transaction({ tenant_id, actor_id }, async (client, actorUuid) => {
      const inserted = await client.query(
        ...
        [tenant_id, request_id, idempotency_key, fingerprint, intent, JSON.stringify({ entities })],
      );
      if (inserted.rowCount === 0) {
        ...
        if (previous.rows[0]?.request_fingerprint !== fingerprint) throw idempotencyConflict();
        if (previous.rows[0]?.status !== "committed") throw conflict("command with this idempotency key is still pending");
      ...
      if (intent === "crm.deal.update_stage") {
        ...
        if (!UUID.test(dealId ?? "")) throw conflict("deal id must be a UUID in PostgreSQL mode");
        ...
        if (updated.rowCount !== 1) throw conflict("deal was not found or its version changed");
        result = { action: "updated", resource: { type: "deal", id: updated.rows[0].id }, aggregate_version: Number(updated.rows[0].version) };
        ...
        const created = await client.query(
          ...
          [tenant_id, entities.customer?.name ?? "Unknown", entities.customer?.preferred_language ?? "en-US"],
        );
        result = { action: "created", resource: { type: "customer", id: created.rows[0].id }, aggregate_version: Number(created.rows[0].version) };
      } else {
        result = { action: "read_only", resource: null, aggregate_version: 0 };
      }
      ...
      await this.#writeAuditAndOutbox(client, { tenant_id, actorUuid, request_id, intent, result });
      return result;
    });
  }

  async createReview({ tenant_id, actor_id, request_id, idempotency_key, request_fingerprint, understanding }) {
    return await this.#transaction({ tenant_id, actor_id }, async (client, actorUuid) => {
      const inserted = await client.query(
        `insert into crm_commands (tenant_id,request_id,idempotency_key,request_fingerprint,intent,payload,status)
         values ($1,$2,$3,$4,$5,$6::jsonb,'needs_review') on conflict (tenant_id,idempotency_key) do nothing returning id`,
        [tenant_id, request_id, idempotency_key, request_fingerprint, understanding.intent, JSON.stringify({ understanding })],
      );
      if (inserted.rowCount === 0) {
        const previous = await client.query(
        ...
        if (previous.rows[0]?.request_fingerprint !== request_fingerprint) throw idempotencyConflict();
        if (!previous.rows[0]) throw conflict("review command exists without its task");
      ...
      const review = await client.query(
        ...
        [tenant_id, inserted.rows[0].id, JSON.stringify(understanding.entities), JSON.stringify({ command_id: inserted.rows[0].id }), expiresAt],
      );
      const result = this.#reviewResult(review.rows[0]);
      const eventResult = { action: "needs_review", resource: { type: "review", id: result.id }, aggregate_version: 1 };
      await this.#writeAuditAndOutbox(client, { tenant_id, actorUuid, request_id, intent: "crm.review.requested", result: eventResult });
      return result;
    });
  }

  async decideReview({ tenant_id, actor_id, review_id, decision, idempotency_key, request_id, correction }) {
    return await this.#transaction({ tenant_id, actor_id }, async (client, actorUuid) => {
      const rawReviewId = review_id.startsWith("rev_") ? review_id.slice(4).replace(/^(?=.{32}$)(.{8})(.{4})(.{4})(.{4})(.{12})$/, "$1-$2-$3-$4-$5") : review_id;
      if (!UUID.test(rawReviewId)) throw conflict("review task id must be a UUID");
      const publicReviewId = this.#reviewId(rawReviewId);
      const requestFingerprint = sha256(JSON.stringify({ review_id: rawReviewId, decision, correction: correction ?? null }));
      ...
      if (previousDecision.rowCount === 1) {
        if (previousDecision.rows[0].request_fingerprint !== requestFingerprint) throw idempotencyConflict();
        return previousDecision.rows[0].result;
      }
      const review = await client.query("select r.*, c.id as command_id, c.intent, c.payload from review_tasks r join crm_commands c on c.id = r.command_id where r.tenant_id=$1 and r.id=$2 for update", [tenant_id, rawReviewId]);
      if (review.rowCount !== 1) throw conflict("review task not found");
      const row = review.rows[0];
      if (row.status !== "open") {
        const closedFingerprint = sha256(JSON.stringify({ review_id: rawReviewId, decision: row.decision?.decision, correction: row.decision?.correction ?? null }));
        if (closedFingerprint !== requestFingerprint) throw conflict("review task is already decided");
        const result = { review_id: publicReviewId, status: row.status, decision: row.decision };
        await client.query(
          ...
          [tenant_id, rawReviewId, idempotency_key, requestFingerprint, JSON.stringify(result)],
        );
        return result;
      }
      if (decision === "reject") {
        const rejected = { decision, actor_id: actorUuid, request_id, decided_at: now(), correction: correction ?? null };
        await client.query("update review_tasks set status='rejected', decision=$3::jsonb, updated_at=now() where tenant_id=$1 and id=$2", [tenant_id, rawReviewId, JSON.stringify(rejected)]);
        await this.#writeAuditAndOutbox(client, { tenant_id, actorUuid, request_id, intent: "crm.review.rejected", result: { action: "rejected", resource: { type: "review", id: publicReviewId }, aggregate_version: 2 } });
        const result = { review_id: publicReviewId, status: "rejected", decision: rejected };
        await client.query(
          `insert into review_decisions (tenant_id,review_id,idempotency_key,request_fingerprint,result)
      ...
      let result = { action: "approved", resource: { type: "review", id: publicReviewId }, aggregate_version: 2 };
      if (row.intent === "crm.customer.create") {
        const created = await client.query("insert into customers (tenant_id,name,preferred_language) values ($1,$2,$3) returning id,version", [tenant_id, entities.customer?.name ?? "Approved customer", entities.customer?.preferred_language ?? "en-US"]);
        result = { action: "created", resource: { type: "customer", id: created.rows[0].id }, aggregate_version: Number(created.rows[0].version) };
      }
      const approved = { decision, actor_id: actorUuid, request_id, decided_at: now(), correction: correction ?? null, result };
      await client.query("update review_tasks set status='approved', decision=$3::jsonb, updated_at=now() where tenant_id=$1 and id=$2", [tenant_id, rawReviewId, JSON.stringify(approved)]);
      await client.query("update crm_commands set status='committed', result=$3::jsonb, committed_at=now() where tenant_id=$1 and id=$2", [tenant_id, row.command_id, JSON.stringify(result)]);
      await this.#writeAuditAndOutbox(client, { tenant_id, actorUuid, request_id, intent: "crm.review.approved", result });
      const response = { review_id: publicReviewId, status: "approved", decision: approved };
      await client.query(
        `insert into review_decisions (tenant_id,review_id,idempotency_key,request_fingerprint,result)
         values ($1,$2,$3,$4,$5::jsonb)`,
        [tenant_id, rawReviewId, idempotency_key, requestFingerprint, JSON.stringify(response)],
      );
      return response;
    });
  }

  async beginInteraction({ tenant_id, actor_id, request_id, idempotency_key, request_fingerprint, input_type, input_payload }) {
    return await this.#transaction({ tenant_id, actor_id }, async (client, actorUuid) => {
      ...
      if (!row || row.request_fingerprint !== request_fingerprint) throw idempotencyConflict();
      if (row.status === "completed" || row.status === "needs_review") {
        return { replay: true, response: this.cipher.decrypt(row.response_ciphertext, `${tenant_id}:response`), http_status: row.http_status };
      }
  ...
  async checkpointInteraction({ tenant_id, actor_id, request_id, transcript, understanding, provider_invocations = [], model_versions = {}, latency_ms = {}, input_asset_id }) {
    return await this.#transaction({ tenant_id, actor_id }, async (client) => {
      const transcriptCiphertext = transcript === undefined ? null : this.cipher.encrypt(transcript, `${tenant_id}:transcript`);
      const understandingCiphertext = understanding === undefined ? null : this.cipher.encrypt(understanding, `${tenant_id}:understanding`);
      const updated = await client.query(
        `update voice_interactions set
  ...
  async completeInteraction({ tenant_id, actor_id, request_id, response, http_status }) {
    return await this.#transaction({ tenant_id, actor_id }, async (client) => {
  ...
  async recordInputAsset({ tenant_id, actor_id, request_id, asset, object_key, byte_length, sha256: assetSha256 }) {
    return await this.#transaction({ tenant_id, actor_id }, async (client) => {
      const inserted = await client.query(
  ...
  async claimOutbox({ tenant_id, worker_id, batch_size = 25, lock_timeout_ms = 60_000 }) {
    return await this.#transaction({ tenant_id }, async (client) => {
      ...
      return claimed.rows.map((row) => ({ outbox_id: row.id, attempts: row.attempts, event: this.#event(row, tenant_id) }));
    }, { requireActor: false });
  }
  ...
  async markOutboxPublished({ tenant_id, worker_id, outbox_id }) {
    return await this.#transaction({ tenant_id }, async (client) => {
      ...
      if (updated.rowCount !== 1) throw conflict("outbox event lease was lost before publish acknowledgement");
    }, { requireActor: false });
  }

  async markOutboxFailed({ tenant_id, worker_id, outbox_id, error, max_attempts = 8 }) {
    return await this.#transaction({ tenant_id }, async (client) => {
      const row = await client.query("select attempts from outbox_events where tenant_id=$1 and id=$2 and lock_owner=$3 for update", [tenant_id, outbox_id, worker_id]);
      if (row.rowCount !== 1) throw conflict("outbox event lease was lost before failure acknowledgement");
      ...
      await client.query(
        ...
        [tenant_id, outbox_id, worker_id, attempts, String(error).slice(0, 2000), dead, delaySeconds],
      );
      return { attempts, dead_lettered: dead, retry_in_seconds: dead ? undefined : delaySeconds };
    }, { requireActor: false });
  }
  ...
  #reviewResult(row) {
    return { id: this.#reviewId(row.id), reason: row.reason, status: row.status, candidates: row.candidates, expires_at: new Date(row.expires_at).toISOString() };
  }

  #reviewId(id) { return `rev_${String(id).replaceAll("-", "")}`; }

  async #writeAuditAndOutbox(client, { tenant_id, actorUuid, request_id, intent, result }) {
    await client.query(
      ...
      [tenant_id, actorUuid, request_id, intent, result.resource?.type ?? "command", result.resource?.id ?? request_id, sha256(JSON.stringify(result))],
    );
    await client.query(
  ...
  #event(row, tenant_id) {
    return validateEvent({
      specversion: "1.0", id: `evt_${row.id}`, type: row.event_type,
      source: "urn:sumi:voice-crm/crm", subject: `${row.aggregate_type}/${row.aggregate_id}`,
      time: new Date(row.created_at).toISOString(), datacontenttype: "application/json",
      tenant_id, request_id: row.request_id, data: row.payload,
    });
  }
  ...
  #ttsKey(key) {
    ...
    const idempotency_key = separator < 0 ? "" : String(key).slice(separator + 1);
    if (!UUID.test(tenant_id) || !idempotency_key) throw Object.assign(new Error("PostgreSQL TTS key must be tenant UUID plus idempotency key"), { code: "FORBIDDEN" });
  ...
  async replayTts(key, fingerprint) {
    ...
    return await this.#transaction(identity, async (client) => {
      const row = await client.query("select request_fingerprint,result from tts_requests where tenant_id=$1 and idempotency_key=$2 for update", [identity.tenant_id, identity.idempotency_key]);
      if (row.rowCount === 0) return undefined;
      if (row.rows[0].request_fingerprint !== fingerprint) throw idempotencyConflict();
      return row.rows[0].result;
    }, { requireActor: false });
  }

  async recordTts(key, fingerprint, result, { tenant_id, actor_id, request_id, object_key, byte_length, sha256: assetSha256 } = {}) {
    const fromKey = this.#ttsKey(key);
    if (tenant_id !== fromKey.tenant_id) throw Object.assign(new Error("TTS tenant does not match idempotency key"), { code: "FORBIDDEN" });
    return await this.#transaction({ tenant_id, actor_id }, async (client, actorUuid) => {
      const existing = await client.query("select request_fingerprint,result from tts_requests where tenant_id=$1 and idempotency_key=$2 for update", [tenant_id, fromKey.idempotency_key]);
      if (existing.rowCount === 1) {
        if (existing.rows[0].request_fingerprint !== fingerprint) throw idempotencyConflict();
      ...
      const media = await client.query(
        ...
        [tenant_id, request_id, object_key ?? result.url ?? `tts/${tenant_id}/${assetId}`, assetId, result.mime_type ?? "application/octet-stream", byte_length ?? 0, assetHash, result.duration_ms ?? null, result.status ?? "ready", result.expires_at ?? new Date(Date.now() + 86_400_000).toISOString()],
      );
      const assetCreated = media.rowCount === 1;
      const mediaRow = media.rows[0] ?? (await client.query(
      ...
      const storedAsset = mediaRow.external_asset_id === assetId
        ? result
        : { ...result, asset_id: mediaRow.external_asset_id, url: `/v1/assets/${mediaRow.external_asset_id}` };
      const inserted = await client.query(
        ...
        [tenant_id, actorUuid, request_id, fromKey.idempotency_key, fingerprint, mediaRow.id, JSON.stringify(storedAsset)],
      );
      if (inserted.rowCount === 0) {
        const race = await client.query("select request_fingerprint,result from tts_requests where tenant_id=$1 and idempotency_key=$2 for update", [tenant_id, fromKey.idempotency_key]);
        if (race.rows[0]?.request_fingerprint !== fingerprint) throw idempotencyConflict();
        return race.rows[0]?.result;
      }
      const persisted = inserted.rows[0].result;
      if (assetCreated) {
        await this.#writeAuditAndOutbox(client, { tenant_id, actorUuid, request_id, intent: "tts.asset.created", result: { action: "created", resource: { type: "asset", id: persisted.asset_id }, aggregate_version: 1 } });
      }
      return persisted;
    });
  }
...
export function createPostgresStore(options) { return new PostgresCrmStore(options); }
