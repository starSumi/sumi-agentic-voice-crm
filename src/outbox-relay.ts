...
import { createControlEngine, type ControlEngine } from "./control/index.ts";

type AnyRecord = Record<string, any>;
type OutboxConfig = {
  tenantIds: string[]; target: string; bearerToken?: string; hmacSecret?: string;
  batchSize: number; maxAttempts: number; lockTimeoutMs: number; pollIntervalMs: number;
};
type OutboxStore = AnyRecord;
type FetchImplementation = (...args: any[]) => Promise<any>;
...
export function outboxConfig(env: NodeJS.ProcessEnv = process.env): OutboxConfig {
  ...
  if (env.APP_ENV === "production" && (env.OUTBOX_HMAC_SECRET?.length ?? 0) < 32) throw new Error("OUTBOX_HMAC_SECRET must contain at least 32 characters");
  return {
    tenantIds,
...
export class OutboxRelay {
  ...
  readonly onResult: (result: AnyRecord) => void;
  readonly control: ControlEngine;
  constructor({ store, config, fetchImpl = fetch, workerId = `relay-${randomUUID()}`, onResult = () => {}, control = createControlEngine() }: { store: OutboxStore; config: OutboxConfig; fetchImpl?: FetchImplementation; workerId?: string; onResult?: (result: AnyRecord) => void; control?: ControlEngine }) {
  ...
  async runOnce({ signal }: { signal?: AbortSignal } = {}): Promise<AnyRecord> {
    ...
    for (const tenant_id of this.config.tenantIds) {
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        try {
          ...
          const candidate = error && typeof error === "object" ? error as AnyRecord : {};
          if (signal?.aborted || candidate.circuitOpen === true) {
            await this.store.releaseOutboxLeases?.({
              tenant_id,
              worker_id: this.workerId,
              outbox_ids: rows.slice(index).map((pending: AnyRecord) => pending.outbox_id),
            });
            if (signal?.aborted) throw signal.reason ?? error;
            break;
          }
          const failure = await this.store.markOutboxFailed({ tenant_id, worker_id: this.workerId, outbox_id: row.outbox_id, error: candidate.message ?? error, max_attempts: this.config.maxAttempts });
          result.failed += 1;
          if (failure.dead_lettered) result.dead_lettered += 1;
          this.onResult({ status: failure.dead_lettered ? "dead_lettered" : "retry_scheduled", tenant_id, outbox_id: row.outbox_id, event_id: row.event.id, error: candidate.message ?? String(error) });
        }
      }
    }
    return result;
  ...
  async #publish(event: AnyRecord, signal?: AbortSignal): Promise<void> {
    const body = JSON.stringify(event);
    const headers: Record<string, string> = { "content-type": "application/cloudevents+json", "idempotency-key": event.id };
    ...
    await this.control.run(`outbox.publish.${target.host}`, async (operationSignal: AbortSignal) => {
...
export function verifyOutboxSignature(body: string, signature: string | undefined, secret: string): boolean {
