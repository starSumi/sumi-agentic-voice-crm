...
function policyFor(name) {
  const policy = GUARDIAN_POLICIES[name];
...
export class GuardianDenialGovernor {
  ...
  constructor({ maxTurns = 1_000, idleTtlMs = 30 * 60_000, now = () => Date.now() } = {}) {
  ...
  recordDenial(id, policyName = "standard") {
  ...
  recordNonDenial(id, policyName = "standard") {
    const policy = policyFor(policyName);
    const turn = this.#getTurn(turnId(id));
    turn.policy = policyName;
  ...
  #getTurn(id) {
    ...
    if (!turn) {
      ...
      turn = { consecutive_denials: 0, recent_denials: [], interrupt_triggered: false };
