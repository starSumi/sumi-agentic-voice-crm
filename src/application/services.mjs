import { ERROR_CODES, sha256 } from "../contracts.mjs";
import {
  assertAskCommand,
...
function opaque(value, fallback = "unknown") {
...
function safeResource(resource) {
  ...
  const type = opaque(resource.type, "resource");
  const id = opaque(resource.id, "unknown");
  return `${type}/${id}`;
}
...
function progressEvent(context, ports, type, fields = {}, conversationId) {
  const event = {
    type,
...
async function emitProgress(ports, context, type, fields, conversationId) {
...
function knownErrorCode(error) {
  return error?.code && Object.hasOwn(ERROR_CODES, error.code)
    ? error.code
    : "INVALID_REQUEST";
}

function result(kind, response) {
