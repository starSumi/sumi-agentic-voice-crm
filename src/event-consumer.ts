...
type EventDeliveryStore = {
  claimEventDelivery(input: Record<string, unknown>): Promise<{ duplicate?: boolean; claimed?: boolean }>;
  completeEventDelivery(input: Record<string, unknown>): Promise<unknown>;
  releaseEventDelivery(input: Record<string, unknown>): Promise<unknown>;
};
type EventConsumerOptions = {
  ...
  lease_ms?: unknown;
  signal?: AbortSignal;
  handler?: (event: EventRecord | undefined, context: Record<string, unknown>) => unknown | PromiseLike<unknown>;
};
...
export async function consumeEvent({
  ...
  signal,
  handler,
}: EventConsumerOptions = {}): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    if (errorCode !== "CRM_CONFLICT" && !signal?.aborted) {
