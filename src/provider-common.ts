...
type UnderstandingInput = { transcript: string; locale: string; model: string };
type AudioAssetOptions = { text: string; language: string; voice: string; format: string; mimeType: string; provider: string; model: string };
export type ProviderCallOptions = { locale?: string; contentType?: string; language?: string; voice?: string; format?: string; signal?: AbortSignal };
...
export function trimBaseUrl(value: unknown, fallback: string): string {
  return String(value || fallback).replace(/\/$/, "");
}

export function bearerHeaders(apiKey: unknown, contentType: string | null = "application/json", extra: Record<string, string> = {}): Record<string, string> {
  const headers: Record<string, string> = { authorization: `Bearer ${apiKey ?? ""}`, ...extra };
  if (contentType) headers["content-type"] = contentType;
  return headers;
}

export async function checkedJson(response: ProviderResponse, label = "provider"): Promise<Record<string, any>> {
...
export function assertProviderResponse(response: ProviderResponse, label = "provider"): void {
...
export function parseUnderstanding(raw: unknown, { transcript, locale, model }: UnderstandingInput): ReturnType<typeof understanding> {
  let parsed: any;
...
export function positiveInteger(value: unknown, fallback: number, label: string, { max }: { max?: number } = {}): number {
...
export function canonicalAudioContentType(contentType = "audio/wav"): string {
  return contentType === "audio/x-wav" ? "audio/wav" : contentType;
}

export function validateTtsText(text: unknown, maxChars: number, provider: string): void {
...
export function normalizeTranscript(text: unknown, provider: string, maxChars = 10_000): string {
...
export async function readBoundedBody(response: ProviderResponse, maxBytes: number, label = "provider audio"): Promise<Buffer> {
